...
用于遍历数组，对每个元素执行指定操作，**无返回值**。

```javascript
const users = ['Tom', 'Jerry', 'Bob']
...
适用场景：
- 遍历执行副作用操作（打印、保存、发送请求等）
- 不需要返回值

### map

用于遍历数组并**返回新数组**，新数组由原数组每个元素经回调函数处理后的返回值组成。

```javascript
const users = ['Tom', 'Jerry', 'Bob']

// 返回新数组
const result = users.map(name => `用户-${name}`)
// result = ['用户-Tom', '用户-Jerry', '用户-Bob']
...
两者时间复杂度相同均为 O(n)，但 map 会创建新数组，有额外空间开销。
