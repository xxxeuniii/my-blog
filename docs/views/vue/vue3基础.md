...
ref:可以获取真实的DOM节点,可以获取到子组件实例VC，父组件获取子组件数据

$parent:可以在子组件内部获取到父组件的实例，子组件获取父组件数据
...
provide(提供)与inject(注入),可以实现父组件传递子组件、孙组件数据
